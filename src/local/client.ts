...
export class LocalClient implements MachineBackend {
  ...
  private async call(method: string, path: string, opts: { json?: unknown; body?: Buffer; timeoutMs?: number; signal?: AbortSignal } = {}) {
  ...
  async listMachines(): Promise<MachineView[]> {
    const res = await this.call("GET", `${API}/machines`);
  ...
  async getMachine(name: string): Promise<MachineView> {
    const res = await this.call("GET", `${API}/machines/${encodeURIComponent(name)}`);
  ...
  async createMachine(opts: CreateOptions): Promise<MachineView> {
    ...
    const res = await this.call("POST", `${API}/machines`, { json: body });
  ...
  async startMachine(name: string): Promise<MachineView> {
    // A start pulls the image inside the guest; give it the ready budget.
    const res = await this.call("POST", `${API}/machines/${encodeURIComponent(name)}/start`, { json: {}, timeoutMs: 300_000 });
  ...
  async stopMachine(name: string): Promise<MachineView> {
    const res = await this.call("POST", `${API}/machines/${encodeURIComponent(name)}/stop`, { json: {}, timeoutMs: 120_000 });
  ...
  async deleteMachine(name: string): Promise<{ deleted: string }> {
    const res = await this.call("DELETE", `${API}/machines/${encodeURIComponent(name)}`, { timeoutMs: 120_000 });
  ...
  async exec(name: string, req: ExecOptions, clientTimeoutMs?: number): Promise<ExecResult> {
  ...
  async readFile(name: string, path: string): Promise<Buffer> {
    const res = await this.call("GET", `${API}/machines/${encodeURIComponent(name)}/files/${encodeURIComponent(path)}`);
    if (res.status !== 200) throw apiError(res, `read ${path} in ${name}`);
    return res.body;
  }

  async writeFile(name: string, path: string, content: Buffer): Promise<{ path: string; size: number }> {
    const res = await this.call("PUT", `${API}/machines/${encodeURIComponent(name)}/files/${encodeURIComponent(path)}`, { body: content });
  ...
  async logs(name: string, tail: number): Promise<string[]> {
    ...
    const res = await this.call("GET", `${API}/machines/${encodeURIComponent(name)}/logs?tail=${tail}&follow=false`, { timeoutMs: 15_000 });
  ...
  async pullImage(name: string, image: string): Promise<ImageInfo> {
    const res = await this.call("POST", `${API}/machines/${encodeURIComponent(name)}/images/pull`, { json: { image }, timeoutMs: 600_000 });
